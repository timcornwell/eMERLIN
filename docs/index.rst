...
- A core set of simple functions are located in erp/functions/imaging_steps.py. These are convenient wrappers around
RASCIL functions. The input parameters are passed by a dictionary called eMRP that is read from a JSON format
file located in the local directory.
- A pipeline function runs these in a predetermined sequence, roughly load ms, average, imaging with selfcalibration,
save results.
